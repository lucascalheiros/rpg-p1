...
int battle_run(){
    ...
    MONSTRO mons[] = {//declaração do nome, ataque, vida e exp dropada dos monstros(base)
        "Goblin" ,  7,  25,   5,  3,
        "Orc"    , 25,  50,  17, 5,
        "Zombie" , 30, 100,  50, 20,
    ...
    SAMPLE* pot = load_sample("sound/pot.wav");
     ...
     if(mapa[mapa_id][(pos_y+28)/PIXELS][(pos_x+13)/PIXELS] == BOSS){
        n_mons = 0;
        battle_m = load_bitmap(name_batt_pics[6],NULL);
        MIDI* battle_st = load_midi(name_batt_st[6]);
        bmons[0] = mons[6];
        mob_pic[0] = load_bitmap( name_mob_pics[6], NULL );
        mob_dead_pic[0] = load_bitmap( name_dead_mob_pics[6], NULL );
        mobs_id[0]=6;
     }
        ...
        while(contador > 0 && (!fechar && battle)){
            ...
            if(stat.LIFE > 0 && verif_wl_var == BATTLE && turn_counter == MEU_TURNO){
                switch(battle_var){
                    ...
                    for( i = 0; i < 3; i++ ){ //verifica se os monstros morreram, caso algum tenha morrido seu verificador é igualado a 0, e os itens dropados são recebidos
                        if( bmons[i].LIFE <= 0 ){
                            bmons[i].LIFE = 0;
                            if(verif_mons[i]){
                                verif_mons[i] = 0;
    ...
    if(mapa[mapa_id][(pos_y+28)/PIXELS][(pos_x+13)/PIXELS] == BOSS && verif_mons[0] == 0 ){
        mapa[mapa_id][(pos_y+28)/PIXELS][(pos_x+13)/PIXELS] = 2;
    }
